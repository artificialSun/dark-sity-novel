...
# словарь с целевым временем в часах для каждой работы. вероятно, будет перенесен в другой блок    
    $ workhours_in_day={
        "morg": 8, #должно быть кратно 2
        "shop": 8  #должно быть кратно 2
    }
...
init python:
    ...
    def generate_morg(): 
        ...
        for i in range(3):
            ...
            morg_note[i]=renpy.random.choice([_("УНЕСТИ"),_("УНЕСТИ"),_("УНЕСТИ"),_("УНЕСТИ"),_("УНЕСТИ"),_("ЗАПИСАТЬ В АНКЕТУ")])
...
init python:
    ...
    def morg_list_premia():
        ...
        if mistakes == 0:
            ...
            #renpy.say(boss, "По-моему, у тебя проблемы. Будь внимательнее!")
            renpy.notify("Вы совершаете много ошибок. Начальник недоволен.")
...
label start_game_morg:
    ...
    if my_can_skip_work["morg"]:  #проверяем, первый раз игрок на эту работу пришел или нет
        ...
        "Моя задача унести все тела, а для некоторых нужно правильно указать имя в анкете." 
          
        while work_hours < workhours_in_day["morg"]:                        
            $ reload_morg_list() 
            show screen morg_mini_game with dissolve
            $ work_hours += 2
            "{cps=120}{color=#2ad9f8}Записывай имена только для тех, для кого указано 'занести в анкету'. Остальных можно просто унести.{/color}{/cps}" 
            pause(0.01)
        ...
        if mistakes_total == 0:
            ...
            $ zp += 50
            $ change_stat("g", 2)
        elif mistakes_total > 2:
            if my_boss_kind == boss_kind["bad"]:
                boss "Совсем охренел?! Ошибка на ошибке! Анкеты пустые и кривые!"
...
label morg_first_day:
    ...
    "Здание полицейского управления почти в самом центре города, недалеко от больницы."
    "В подвале лаборатория криминалистов и морг. Я забираю у них отработанный материал и заношу некоторые данные в картотеку."
...
label job_first_day_boss_dialog1:        
    ...
    if my_boss_kind == boss_kind["good"]:
        if t < -20:
            ...
            boss "Почему ты задержался, [my_name]? У тебя какие-то проблемы? Тебе придется все равно отработать все время"
        elif t > 0:
            boss "Опаздываешь, [my_name]. Не хорошо. Очень надеюсь, что у тебя есть на это причины."
        else:
            boss "Доброе утро, [my_name]. Ты как раз вовремя, там новеньких завезли."
        if my_choises["ep001_whiskey_drink"]:
            boss "Мне кажется, или от тебя пахнет спиртом?"
            boss "*неодобрительно качает головой*"
            $ change_stat("g", -1)
            $ change_stat("b", 1)
        ...
        if t > 15:
            ...
            boss "Что, в этот раз притащил свою задницу вовремя? Там тебя уже ждет работенка."
        if my_choises["ep001_whiskey_drink"]:
            boss "*принюхивается*"
            boss "Ты уже за воротник заложил с утра пораньше??? Алкоголик. Чтоб тебя."
            boss "Как только найду кого-то нового, сразу уволю!"
            "Он всегда так. Не обращаю внимания."
    hide boss_img with dissolve
    return


#XX здесь будет дополнительная работа
label morg_dop_work:
    show boss_img with dissolve

    boss "Закончил?"
    if my_boss_kind == boss_kind["good"]:
        boss "[my_name], не мог бы ты задержаться?"
    elif my_boss_kind == boss_kind["bad"]:        
        boss "Что-то ты быстро освободился смотрю!"
        me "Я отработал полное время."
        boss "Хм. Хм. Похоже на то. Черт с тобой."
        boss "Слушай, давай-ка ты немного задержишься, а?"
        me "А сколько мне заплатят?"
    hide boss_img with dissolve
    return
